...
import { createRandom, groundY, instanceMatrix } from "./placement";
...
/** Low, softly irregular hedgerows along the given polylines (world x/z),
 * with a few wildflowers at their foot — planting that makes a place feel
 * grown into its land rather than placed on it. `surfaceY` gives the
 * ground height they stand on (defaults to the terrain; a stage can pass
 * e.g. a terrace top instead). */
export function Hedgerows({
  ...
  /** Distance between bushes along the line. */
  spacing?: number;
  /** Bush size multiplier. */
  size?: number;
}) {
    ...
    for (const line of lines) {
      for (let i = 0; i < line.length - 1; i++) {
        ...
        for (let s = 0; s < steps; s++) {
          ...
          const r = (0.075 + random() * 0.04) * size;
          bushes.push(instanceMatrix(x, surfaceY(x, z) + r * 0.55, z, random() * 6, r * 1.1, r * 0.85, r));
          bushColors.push(HEDGE_COLORS[Math.floor(random() * HEDGE_COLORS.length)]);
          if (random() < 0.45) {
            const fx = x + (random() - 0.5) * 0.2;
            const fz = z + (random() - 0.5) * 0.2;
            flowers.push(instanceMatrix(fx, surfaceY(fx, fz) + 0.02, fz, 0, 0.011 + random() * 0.005));
  ...
  }, [lines, seed, surfaceY, spacing, size]);
